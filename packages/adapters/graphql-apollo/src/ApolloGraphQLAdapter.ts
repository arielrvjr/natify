...
export class ApolloGraphQLAdapter implements GraphQLPort {
  ...
  constructor(config: ApolloAdapterConfig) {
    ...
    this.client = new ApolloClient({
      ...
      defaultOptions: {
        watchQuery: {
          fetchPolicy: this.config.cacheEnabled ? 'cache-and-network' : 'network-only',
        },
        query: {
          fetchPolicy: this.config.cacheEnabled ? 'cache-first' : 'network-only',
        },
      },
    });
  }
  ...
  async query<T = any>(query: string, options?: GraphQLOptions): Promise<GraphQLResult<T>> {
    try {
      ...
      // Convertir errores de Apollo a NatifyError
